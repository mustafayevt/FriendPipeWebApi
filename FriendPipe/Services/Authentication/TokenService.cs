...
namespace FriendPipe.Services
{
    public class TokenService : ITokenService
    {
        ...
        {
            var tokenByte = Encoding.UTF8.GetBytes(_configuration.GetSection("JwtSettings")["Secret"]);
            var key = new SymmetricSecurityKey(tokenByte);
            var credentials = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);

            if (!double.TryParse(_configuration.GetSection("JwtSettings")["Expires"], out double expireTime))
                expireTime = 1;
